...
likingResults1 <- data.frame(
  ...
  r_value = sapply(1:5, function(i) abs(qnorm(wilcox.test(as.matrix(alternativeLiking1[,i]),as.matrix(rewardLiking1[,i]))$p.value/2))/sqrt(nrow(alternativeLiking1[,i]))),
  p_value = sapply(1:5, function(i) wilcox.test(as.matrix(alternativeLiking1[,i]),as.matrix(rewardLiking1[,i]))$p.value)
)
...
getTableAllRvsA<-function(rewardLiking,alternativeLiking){
  ...
  for (iTask in 1:5) {
    for (jTask in 1:5) {
      ...
      t_test_result <- wilcox.test(diff,mu = 0)
      matP[iTask, jTask] <- t_test_result$p.value
      matSE[iTask, jTask] <- sd(diff)/sqrt(length(diff))
...
getTableAllRvsN<-function(rewardLiking,alternativeLiking){
  ...
  for (iTask in 1:4) {
      ...
      matM2[iTask,2]<- sd(diff)/sqrt(length(diff))
      matM2[iTask,3] <- abs(qnorm(t_test_result$p.value/2))/sqrt(length(diff))
      matM2[iTask,4] <- t_test_result$p.value[1]
  }
  # convert matM2 to a data frame and add column/row names
  matM2_df <- data.frame(matM2)
  colnames(matM2_df) <- c("Reward (M)", "Reward (SE)", "r","p-value")
...
wantingResults1 <- data.frame(
  ...
  SE_Reward_Rating = sapply(Y, sd) / sqrt(nrow(Y)),
   r_value = sapply(1:5, function(i) abs(qnorm(wilcox.test(as.matrix(Y[,i]),mu = 50)$p.value/2))/sqrt(nrow(Y))),
  p_value = sapply(1:5, function(i) wilcox.test(as.matrix(Y[,i]),mu = 50)$p.value)
)
...
likingResults2 <- data.frame(
  ...
  r_value = sapply(1:5, function(i) abs(qnorm(wilcox.test(as.matrix(Y[,i]),as.matrix(Z[,i]))$p.value/2))/sqrt(nrow(alternativeLiking1[,i]))),
  p_value = sapply(1:5, function(i) wilcox.test(as.matrix(Y[,i]),as.matrix(Z[,i]))$p.value)
)
...
# Create the violin plot with overlaid data points and boxplot ,adjust=0.3
  ...
  labs(x = "", y = "Value")
  if(text==0){
  vp<-vp0+theme(legend.position = "none",axis.text.y=element_text(size=7,colour="black"),axis.text.x=element_blank(),plot.margin = margin(0, 0, 0, 0, "pt"))
  } else {
   vp<-vp0+ theme(legend.position = "none",axis.text=element_text(size=7,colour="black"),plot.margin = margin(0, 0, 0, 0, "pt"))
  }
  return(vp)
}
...
figure2A<-p1 + p2 + p3 + plot_layout(ncol = 1)
# Experiment 2
df_long<-get_df_long(lStim2)
p1<-violin_plot(df_long[[1]],0,0)
p2<-violin_plot(df_long[[2]],50,0)
p3<-violin_plot(df_long[[3]],50,1)
figure2B<-p1 + p2 + p3 + plot_layout(ncol = 1)
# Experiment 3
df_long<-get_df_long(lStim3)
p1<-violin_plot(df_long[[1]],0,0)
p2<-violin_plot(df_long[[2]],50,0)
p3<-violin_plot(df_long[[3]],50,1)
figure2C<-p1 + p2 + p3 + plot_layout(ncol = 1)

ggsave(file = "figure2A.pdf",plot = figure2A, dpi = 600, width = 3, height = 3.8)
ggsave(file = "figure2B.pdf",plot = figure2B, dpi = 600, width = 3, height = 3.8)
ggsave(file = "figure2C.pdf",plot = figure2C, dpi = 600, width = 3, height = 3.8)
...
ggp<- ggplot(df)+
  ...
  geom_line(aes(x=nb,y=sVE),stat="identity",color="grey",size=1)+
  geom_point(aes(x=nb,y=sVE),size=2,shape=21,color="grey")+
...
ggp
figure4A_left<-ggp
ggsave(file = "figure4A_left.pdf",plot = figure4A_left, dpi = 600, width = 1, height = 1)

# plot the loadings  
# par(mar=c(6,12,4,4))
fontsize=6
pdf("figure4A_centre.pdf", width = 3, height = 4)
barplot(height=-fa.none$loadings[,1], horiz=T,main="Questionnaires 1st Factor",xlab="Factor loadings",
names.arg=c("PHQ","AES-Behaviour","AES-Cognition","AES-Emotion","AES-other", "SHAPS-Social","SHAPS-Food","SHAPS-Perceptual","SHAPS-PastTime","DOPS-PersonnalAchievement","DOPS-Sex","DOPS-Social","DOPS-Perceptual","LifeHappiness","CurrentHappiness"),las=1,cex.axis=fontsize/12, cex.names=fontsize/12,cex.main=fontsize/12,cex.lab=fontsize/12,xlim=c(-0.8,0.8))
dev.off()
...
ggp
figure4B_left<-ggp
ggsave(file = "figure4B_left.pdf",plot = figure4B_left, dpi = 600, width = 1, height = 1)

# plot the loadings  
# par(mar=c(6,12,4,4))
fontsize=6
pdf("figure4B_centre.pdf", width = 3, height = 4)
barplot(height=-fa.none$loadings[,1], horiz=T,main="Questionnaires 1st Factor",xlab="Factor loadings",
names.arg=c("PHQ","AES-Behaviour","AES-Cognition","AES-Emotion","AES-other", "SHAPS-Social","SHAPS-Food","SHAPS-Perceptual","SHAPS-PastTime","DOPS-PersonnalAchievement","DOPS-Sex","DOPS-Social","DOPS-Perceptual","LifeHappiness","CurrentHappiness"),las=1,cex.axis=fontsize/12, cex.names=fontsize/12,cex.main=fontsize/12,cex.lab=fontsize/12,xlim=c(-0.8,0.8))
dev.off()
...
ggp
figure4C_left<-ggp
ggsave(file = "figure4C_left.pdf",plot = figure4B_left, dpi = 600, width = 1, height = 1)

# plot the loadings  
# par(mar=c(6,12,4,4))
fontsize=6
pdf("figure4C_centre.pdf", width = 3, height = 4)
barplot(height=-fa.none$loadings[,1], horiz=T,main="Questionnaires 1st Factor",xlab="Factor loadings",
names.arg=c("PHQ","AES-Behaviour","AES-Cognition","AES-Emotion","AES-other", "SHAPS-Social","SHAPS-Food","SHAPS-Perceptual","SHAPS-PastTime","DOPS-PersonnalAchievement","DOPS-Sex","DOPS-Social","DOPS-Perceptual","LifeHappiness","CurrentHappiness"),las=1,cex.axis=fontsize/12, cex.names=fontsize/12,cex.main=fontsize/12,cex.lab=fontsize/12,xlim=c(-0.8,0.8))
dev.off()
...
names.arg=rev(c("Liking Visual","Liking Cognitive","Liking Social","Liking Monetary","Liking Neutral","Wanting Visual","Wanting Cognitive","Wanting Social","Wanting Monetary","Wanting Neutral","Reinforcing Visual","Reinforcing Cognitive","Reinforcing Social","Reinforcing Monetary","Reinforcing Neutral")),las=1,cex.axis=fontsize/12, cex.names=fontsize/12,cex.main=fontsize/12,cex.lab=fontsize/12)
}
...
# plot loadings
pdf("figure3A_right.pdf", width = 2, height = 4)
...
ggsave(file = "figure3A_centre.pdf",plot = figure3A_centre, dpi = 600, width = 1, height = 1)
```
```{r echo=FALSE}
...
# remove outliers
mentalHealth_NoOutlier<-scale(subset(scoreFA_PP1,ioi))
rewardSensitivity_NoOutlier<-scale(subset(scoreFA_R1[,1],ioi))
...
plot<-ggplot(df, aes(x = x1, y = y))+
  ...
  geom_smooth(method = "lm", se = TRUE, color="green4",size=1, fill="#98FB98")+
...
print(plot)
figure4D<-plot
ggsave(file = "figure4D.pdf",plot = figure4D, dpi = 600, width = 2.5, height = 2.5)

#check assumptions
# linearity
ltest<-resettest(linearMod1)
# homoscedasticity
htest<-ncvTest(linearMod1)
#table
lm_assumptions <- data.frame(
  statistic = c(ltest$statistic[[1]], htest$ChiSquare),
  p_value = c(ltest$p.value[[1]], htest$p),
  row.names = c("linearity test", "homoscedasticity test")
)
print(lm_assumptions)
```
...
# plot loadings
pdf("figure3B_right.pdf", width = 2, height = 4)
plot_loadings(fa.none$loadings[,1],1)
dev.off()
plot_loadings(fa.none$loadings[,2],2)
plot_loadings(fa.none$loadings[,3],3)
...
ggsave(file = "figure3B_centre.pdf",plot = figure3B_centre, dpi = 600, width = 1, height = 1)
```
...
mentalHealth_NoOutlier<- scale(subset(scoreFA_PP2,ioi))
rewardSensitivity_NoOutlier<-scale(subset(scoreFA_R2[,1],ioi))
...
print(plot)
figure4E<-plot
ggsave(file = "figure4E.pdf",plot = figure4E, dpi = 600, width = 2.5, height = 2.5)


#check assumptions
# linearity
ltest<-resettest(linearMod2)
# homoscedasticity
htest<-ncvTest(linearMod2)
#table
lm_assumptions <- data.frame(
  statistic = c(ltest$statistic[[1]], htest$ChiSquare),
  p_value = c(ltest$p.value[[1]], htest$p),
...
# plot loadings
pdf("figure3C_right.pdf", width = 2, height = 4)
plot_loadings(fa.none$loadings[,1],1)
dev.off()
plot_loadings(fa.none$loadings[,2],2)
plot_loadings(fa.none$loadings[,3],3)
...
ggsave(file = "figure3C_centre.pdf",plot = figure3C_centre, dpi = 600, width = 1, height = 1)
```
...
mentalHealth_NoOutlier<- scale(subset(scoreFA_PP3,ioi))
rewardSensitivity_NoOutlier<-scale(subset(scoreFA_R3[,1],ioi))
...
mentalHealth<-scoreFA_PP3
rewardSensitivity<-scoreFA_R3[,1]
df<-data.frame(mentalHealth,rewardSensitivity,lDemo3)
linearMod <- lm( mentalHealth ~ rewardSensitivity, data=df,na.action=na.exclude)
...
linearMod2 <- lm(mentalHealth_NoOutlier ~ rewardSensitivity_NoOutlier, data=df)
summary(linearMod3)
summary3 <- getLinearModelTable(linearModDemo3)
summary4 <- getLinearModelTable(linearMod3)
...
#check assumptions
# linearity
ltest<-resettest(linearMod3)
# homoscedasticity
htest<-ncvTest(linearMod3)
#table
lm_assumptions <- data.frame(
  statistic = c(ltest$statistic[[1]], htest$ChiSquare),
  p_value = c(ltest$p.value[[1]], htest$p),
